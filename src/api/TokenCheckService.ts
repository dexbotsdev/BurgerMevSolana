...
class TokenCheckService {
    private async _analyseMarket(result: { totalLPLiquidity: number; transferFee: { pct: any; maxAmount: any; authority: any; }; sellerFeeBasisPoints: string; freezeAuth: boolean; mintAuthority: boolean; tokenName: string; tokenSymbol: string; tokenDecimals: number; quoteMint: string; quoteSymbol: string; quoteDecimals: number; ammLiquidityPct: number; ammLiquidity: string; base: string; liquidityUSDC: number; liquiditySOL: number; holdersCount: number; largeHolderPct: number; mutable: boolean; }, i: number): Promise<any> {
        //@ts-ignore
    ...
    async getPrice(quoteMint: any, result: { totalLPLiquidity: number; transferFee: { pct: any; maxAmount: any; authority: any; }; sellerFeeBasisPoints: string; freezeAuth: boolean; mintAuthority: boolean; tokenName: string; tokenSymbol: string; tokenDecimals: number; quoteMint: string; quoteSymbol: string; quoteDecimals: number; ammLiquidityPct: number; ammLiquidity: string; base: string; liquidityUSDC: number; liquiditySOL: number; holdersCount: number; largeHolderPct: number; mutable: boolean; }) {
       ...
       { 
        ...
        return 1 ;} else if(quoteMint.toString() ==WSOL.mint){
            this.result.quoteSymbol=WSOL.symbol;
        ...
        const resp = await fetch(`https://price.jup.ag/v4/price?ids=${quoteMint}`)
    ...
    result: {
        ...
        transferFee: { pct: any; maxAmount: any; authority: any; };
    ...
    topHolders: import("@solana/web3.js").TokenAccountBalancePair[];
    ...
    constructor(tokenAddress: string) {
        ...
        this.result={
            totalLPLiquidity: 0,
        transferFee: { pct: 0, maxAmount: 0, authority: '' },
    ...
    async analyseToken() {
        ...
        if (this.token.mintAuthority !== null && this.token.mintAuthority.toString() !== "11111111111111111111111111111111") {
            ...
            if (!authority.value) {
                ...
                //@ts-ignore
                this.result.mintAuthority = false  
            } else { 
                this.result.mintAuthority = true ;  
            }
        }
        ...
        this.result.sellerFeeBasisPoints = token.mint.supply.basisPoints.toString();
